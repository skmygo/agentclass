...
@app.cell
def _(mo):
    my_hits = mo.ui.slider(
        1, 9, 1, value=6,
        label="9 次抽樣裡答對幾次（1️⃣ 的實測是 6 次）", show_value=True,
    )
    mo.vstack([mo.md("**你的實驗區**——用實測的答對率算「該投幾票」。"), my_hits])
    return (my_hits,)


@app.cell
def _(maj_acc, mo, my_hits):
    _p = my_hits.value / 9
    _rows = "\n    ".join(
        f"| 投 {_n} 次 | {maj_acc(_p, _n):.1%} |" for _n in (1, 3, 9, 21, 41)
    )
    _gain = maj_acc(_p, 41) - maj_acc(_p, 21)
    mo.md(
        f"""
    單次答對率 p = {my_hits.value}/9 = **{_p:.1%}**：

    | 投幾票 | 多數決答對率 |
    | --- | --- |
    {_rows}

    從 21 票到 41 票只多 **{_gain:.1%}**，算力卻加倍——
...
def _(mo):
    ...
    """
            ...
            "💡 LEVEL 2 參考解答": mo.md(
                r"""
    實驗區的拉桿留在 6（＝實測的 p = 6/9 ≈ 66.7%）：

    | 投幾票 | 1 | 3 | 9 | 21 | 41 |
    |---|---|---|---|---|---|
    | 多數決答對率 | 66.7% | 74.1% | 85.5% | 94.4% | **98.6%** |
