...
- 全系列 8 課的互動模式改成 **app**（`content/local-llm/lesson-mode`）：右欄隱藏程式碼，
  只留說明／互動元件／輸出。理由——右欄是**教學模擬**（十六維玩具注意力層、計費模擬、
  取樣模擬），程式碼不是教學標的，攤開只會讓學員誤以為右邊在跑真的推論。
- 連帶改掉的東西（同一系列日後重驗時別改回去）：
  - 開場白從「每一格程式碼都可以直接修改、立即重跑」改成「每個實驗都有滑桿與選項可以拉」。
  - 五課的自由編碼「你的實驗區」改寫成互動元件：ollama-vs-vllm＝prefix caching 服務倍率、
    prompt-caching＝四根拉桿的帳單、sampling-params＝長文 penalty 前後段對照、
    kv-offload＝主體拉桿的總結卡、speculative-decoding＝K 掃描、lora-basics＝α/r 對照、
    llm-observability＝抖動情境的告警規則、kv-cache＝兩句話的共用前綴。
  - 挑戰題與解答裡的程式碼區塊全部換成「拉哪根桿、看哪個數字」，數字重新實算過。
- **實測發現：lora-basics 原 LEVEL 3 解答的預期是錯的**——把 `_w` 四個權重改成全 1.0，
  r=4 的還原誤差**不會上升**（都是 4.6%）：方向數還是 4，r=4 吃得下。真正讓誤差爆掉的
  是雜訊（十倍雜訊 → 40.7%）。已改寫成三選項 dropdown 並附實算表。
