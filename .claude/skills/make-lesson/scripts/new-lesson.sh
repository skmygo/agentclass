...
ID="$1"; TITLE="$2"; TOPIC="$3"; TOPIC_NAME="$4"; MODE="${5:-}"
...
接下來輪到你（頁面用 Edit 改內容區，別整檔重寫）：
  1. content/$TOPIC/$ID/${ID}_ext.py — 課程內容（唯一的程式版本：大量 md 解說、自成完整教材、
     emoji 章節錨點；PEP 723 依賴、GPU 檢查 cell 不需要就刪）
  2. content/$TOPIC/$ID/index.html   — hero／各 section／molab 面板步驟／endnav／語義色
  3. content/$TOPIC/$ID/smoke-test.mjs — H1_TEXT 確認與頁面 h1 一致
  4. wiring — 主題頁課卡、首頁主題卡課數、前一課的「下一課」連結
驗證：uv run marimo export html --sandbox content/$TOPIC/$ID/${ID}_ext.py -o check_ext.html
EOF
else
  cat <<EOF
接下來輪到你（頁面用 Edit 改內容區，別整檔重寫）：
